...
public class AuthServerConfig {
	...
	public SecurityFilterChain authServerFilterChain(HttpSecurity http) throws Exception {
		...
		return http.formLogin(Customizer
									.withDefaults()).build();
	}
	...
	public OAuth2TokenCustomizer<JwtEncodingContext> jwtBuildCustomizer() {
		return (context) -> {
			...
			usuarioSistema.getAuthorities().forEach(grantedAuthority -> authorities.add(grantedAuthority.getAuthority()));
	...
	@Bean
	public JWKSource<SecurityContext> jwtSource(JWKSet jwkSet) {
